...
class PersistenciaLocalIntegrationTest {
    ...
    void deveExporRotasDeLeituraComFallbackOuUsoLocalPorCompletudeDaShadow() throws Exception {
        ...
        mockMvc.perform(get("/actuator/health/professorShadowPersistence"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.details.listarCutoverEnabled").value(false))
                ...
                .andExpect(jsonPath("$.details.shadowReadRoutes.listar.cutoverEnabled").value(false))
                .andExpect(jsonPath("$.details.shadowReadRoutes.listar.rollbackStrategy").value("disable_property"))
                .andExpect(jsonPath("$.details.shadowReadRoutes.listar.localCutoverBlockedTotal").value(0.0))
