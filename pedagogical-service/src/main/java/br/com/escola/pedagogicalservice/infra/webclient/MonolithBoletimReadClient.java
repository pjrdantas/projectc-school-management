...
@Component
public class MonolithBoletimReadClient implements BoletimReadPort {

    private final RestClient restClient;
    private final MeterRegistry meterRegistry;

    public MonolithBoletimReadClient(
        ...
        try {
            ...
            if (exception.getStatusCode().value() == 404) {
                meterRegistry.counter("pedagogical.monolith.requests", "route", "boletimPorMatricula", "result", "not_found")
                        .increment();
                throw new PedagogicalServiceResourceNotFoundException("Boletim nao encontrado para a matricula informada");
            }
        ...
        try {
            ...
            if (exception.getStatusCode().value() == 404) {
                meterRegistry.counter("pedagogical.monolith.requests", "route", "boletimFechamentosPorMatricula", "result", "not_found")
                        .increment();
                throw new PedagogicalServiceResourceNotFoundException("Fechamentos de boletim nao encontrados para a matricula informada");
            }
