...
@Component
public class OrigemAtualHistoricoEscolarWriteClient implements HistoricoEscolarWritePort {

    private final RestClient restClient;
    private final MeterRegistry meterRegistry;

    public OrigemAtualHistoricoEscolarWriteClient(RestClient monolithPedagogicalRestClient, MeterRegistry meterRegistry) {
    ...
    public ResponseEntity<String> criar(String authorization, InternalRequestContext context, String requestBody) {
        try {
            ...
            if (exception.getStatusCode().value() == 404) {
                ...
                throw new RecursoNaoEncontradoException("Dependencia do historico escolar nao encontrada");
            }
        ...
        try {
            ...
            if (exception.getStatusCode().value() == 404) {
                meterRegistry.counter("pedagogical.monolith.requests", "route", "historicoAtualizar", "result", "not_found")
                        .increment();
                throw new RecursoNaoEncontradoException("Historico escolar nao encontrado para atualizacao");
            }
