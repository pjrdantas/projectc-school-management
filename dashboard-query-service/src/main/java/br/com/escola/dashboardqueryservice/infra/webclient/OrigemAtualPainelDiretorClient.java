...
@Component
public class OrigemAtualPainelDiretorClient implements PainelDiretorPort {

    private final RestClient restClient;

    public OrigemAtualPainelDiretorClient(RestClient dashboardQueryMonolithRestClient) {
    ...
    public PainelDiretorResponse consultar(String authorization, InternalRequestContext context) {
        try {
            return restClient.get()
                    ...
                    .retrieve()
                    .body(PainelDiretorResponse.class);
        } catch (RestClientResponseException exception) {
            if (exception.getStatusCode().value() == 404) {
                throw new PainelQueryServiceResourceNotFoundException(
                        "Painel diretor nao encontrado no monolito");
            }
            throw exception;
