...
| `/api/professores/**` e alocacoes por professor/turma | `academic-professor-service` | ownership final decidido no `D2`, mas o runtime ainda usa `LegacyConsultaClient`, `LegacyComandoClient`, `LegacyHealthIndicator` e shadow/local persistence com fallback por sync state; o BFF ainda le parcialmente professor resumo via `people-service` | executar `D9`, reroteando o BFF para o owner oficial e removendo a dependencia shadow/legada | nao restar `PROFESSOR_SHADOW_MONOLITH_BASE_URL`, fallback shadow em producao nem leitura publica de professor apontando para `people-service` |
...
| Migracao e shadow sync de professores | `academic-professor-service` | a migracao operacional recorrente foi removida; permanecem `LegacyHealthIndicator` e sync states shadow como residuos da dependencia legada | concluir `D9`, fechando a superficie shadow remanescente | nenhum runner, reconciliacao runtime ou health `professorShadowMonolith` ativo para operacao normal |
