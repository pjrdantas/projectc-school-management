...
import static br.com.escola.catalog.infra.database.mapper.PersistenciaMapper.toDomain;
import static br.com.escola.catalog.infra.database.mapper.PersistenciaMapper.toEntity;
...
import br.com.escola.catalog.infra.database.mapper.PersistenciaMapper;
...
@Repository
@Transactional
public class PersistenciaAdapter implements
    ...
    public Optional<NivelEnsino> buscarPorId(UUID id) {
        return nivelEnsinoRepository.findById(id).map(PersistenciaMapper::toDomain);
    }
    ...
    public List<NivelEnsino> listarNiveisEnsino() {
        return nivelEnsinoRepository.findAllByOrderByCodigoAsc().stream()
                .map(PersistenciaMapper::toDomain).toList();
    }
    ...
    public Optional<PeriodoLetivo> buscarPeriodoPorId(UUID id, EscolaId escolaId) {
        return periodoRepository.findByIdAndEscolaId(id, escolaId.value()).map(PersistenciaMapper::toDomain);
    }
    ...
    public Optional<Serie> buscarSeriePorId(UUID id, EscolaId escolaId) {
        return serieRepository.findByIdAndEscolaId(id, escolaId.value()).map(PersistenciaMapper::toDomain);
    }
    ...
    public Optional<Turno> buscarTurnoPorId(UUID id) {
        return turnoRepository.findById(id).map(PersistenciaMapper::toDomain);
    }

    @Override
    @Transactional(readOnly = true)
    public Optional<Turno> buscarTurnoPorCodigo(String codigo) {
        return turnoRepository.findByCodigoIgnoreCase(codigo).map(PersistenciaMapper::toDomain);
    }
    ...
    public Optional<Turma> buscarTurmaPorId(UUID id, EscolaId escolaId) {
        return turmaRepository.findByIdAndEscolaId(id, escolaId.value()).map(PersistenciaMapper::toDomain);
    }
