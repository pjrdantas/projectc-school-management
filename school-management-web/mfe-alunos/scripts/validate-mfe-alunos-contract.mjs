...
if (errors.length > 0) {
  console.error('Contrato do mfe-alunos invalido:');
...
console.log(
  `Contrato do mfe-alunos valido: ${manifestItems.length} entradas conferidas entre manifesto, exposes locais, federation.config.js e shell do host.`,
);
...
function validateShellContract(manifestItems, shellStudentRoutes, shellStudentCandidates) {
  return [
    ...
    ...validateShellExtractionCandidate(shellStudentCandidates),
  ];
}
...
function validateShellRoutes(manifestItems, shellStudentRoutes) {
  ...
  for (const item of manifestItems) {
    ...
    if (shellRoute.runtimeRemoteName !== 'mfe-alunos') {
      errors.push(
        `rota ${item.path} do shell deve usar runtimeRemoteName mfe-alunos, encontrado ${shellRoute.runtimeRemoteName}.`,
      );
    }
...
function validateShellExtractionCandidate(shellStudentCandidates) {
  const errors = [];

  if (shellStudentCandidates.length !== 0) {
    errors.push(
      `shell do host nao deve mais declarar candidato de extracao para alunos, encontrados ${shellStudentCandidates.length}.`,
    );
  }
