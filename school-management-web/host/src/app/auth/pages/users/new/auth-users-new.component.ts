...
form=this.fb.group({username:['',Validators.required],nome:['',Validators.required],email:['',[Validators.required,Validators.email]],senhaHash:['',Validators.required],ativo:[true],perfilIds:[[] as string[]]}); ngOnInit(){const id=this.service.currentUserId(); if(!id)return; this.userId.set(id); this.service.buscarUsuario(id).subscribe(u=>this.form.patchValue({...u, senhaHash:'********'}));}
