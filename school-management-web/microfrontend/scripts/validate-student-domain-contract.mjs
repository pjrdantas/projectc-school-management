...
const manifestItems = readArrayObjects(manifestSource, 'STUDENT_DOMAIN_MANIFEST');
const exposes = readExposes(federationSource);
const shellStudentRoutes = readArrayObjects(shellSource, 'SHELL_REMOTE_ROUTES').filter(
  route => route.domain === 'alunos',
);
const shellStudentCandidates = readArrayObjects(
  shellSource,
  'SHELL_EXTRACTION_CANDIDATES',
).filter(candidate => candidate.domain === 'alunos');

const errors = [
  ...validateManifestShape(manifestItems),
  ...validateFederation(manifestItems, exposes),
  ...validateExposeFiles(manifestItems),
  ...validateShellContract(manifestItems, shellStudentRoutes, shellStudentCandidates),
];

if (errors.length > 0) {
  console.error('Contrato interno do dominio alunos invalido:');
...
console.log(
  `Contrato interno de alunos valido: ${manifestItems.length} entradas conferidas entre manifesto, exposes, federation.config.js e shell do host.`,
);
...
function validateManifestShape(manifestItems) {
  ...
  for (const item of manifestItems) {
    requireString(item, 'key', 'item do manifesto alunos', errors);
    requireString(item, 'domain', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
    requireString(item, 'futureRemoteName', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
    requireString(item, 'path', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
    requireString(item, 'exposedModule', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
    requireString(item, 'exposeFilePath', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
    requireString(item, 'exportName', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
    requireString(item, 'routeKind', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
    requireString(item, 'routeRole', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
    requireString(item, 'shellNavigation', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
    requireBoolean(item, 'extractionCandidate', `manifesto alunos ${item.key ?? '<sem-chave>'}`, errors);
  ...
  if (domains.size > 1) {
    errors.push('manifesto alunos deve manter um unico dominio interno.');
  }

  if (!domains.has('alunos')) {
    errors.push('manifesto alunos deve usar o dominio alunos.');
  }

  if (futureRemoteNames.size > 1) {
    errors.push('manifesto alunos deve manter um unico futureRemoteName.');
  }

  if (!futureRemoteNames.has('mfe-alunos')) {
    errors.push('manifesto alunos deve usar futureRemoteName mfe-alunos.');
  }
...
function validateRouteClassification(manifestItems) {
  ...
  for (const item of manifestItems) {
    ...
    if (item.path === 'students/:id/edit') {
      if (item.routeKind !== 'edit') {
        errors.push(`rota ${item.path} deve marcar routeKind como edit.`);
      }
...
function validateExposeFiles(manifestItems) {
  ...
  for (const item of manifestItems) {
    ...
    if (!exportDeclaration) {
      errors.push(`arquivo ${item.exposeFilePath} deve reexportar um componente de ../student-domain.`);
      continue;
    }

    if (exportDeclaration.moduleSpecifier !== '../student-domain') {
      errors.push(
        `arquivo ${item.exposeFilePath} deve reexportar a partir de ../student-domain, encontrado ${exportDeclaration.moduleSpecifier}.`,
      );
    }
...
function validateShellContract(manifestItems, shellStudentRoutes, shellStudentCandidates) {
  return [
    ...validateShellRoutes(manifestItems, shellStudentRoutes),
    ...validateShellExtractionCandidate(manifestItems, shellStudentRoutes, shellStudentCandidates),
  ];
}

function validateShellRoutes(manifestItems, shellStudentRoutes) {
  const errors = [];

  if (shellStudentRoutes.length !== manifestItems.length) {
    errors.push(
      `shell do host deve expor ${manifestItems.length} rotas de alunos, encontradas ${shellStudentRoutes.length}.`,
    );
  }

  for (const item of manifestItems) {
    const shellRoute = shellStudentRoutes.find(route => route.path === item.path);

    if (!shellRoute) {
      errors.push(`shell do host nao declarou a rota ${item.path} para alunos.`);
      continue;
    }
...
function validateShellExtractionCandidate(manifestItems, shellStudentRoutes, shellStudentCandidates) {
  const errors = [];

  if (shellStudentCandidates.length !== 1) {
    errors.push(
      `shell do host deve declarar um unico candidato de extracao para alunos, encontrados ${shellStudentCandidates.length}.`,
    );
    return errors;
  }

  const [candidate] = shellStudentCandidates;
  ...
  const contextualRoutes = manifestItems
    ...
    .map(item => item.path);
  const runtimeRemoteNames = [...new Set(shellStudentRoutes.map(route => route.runtimeRemoteName))];

  if (candidate.currentPlacement !== 'microfrontend') {
    errors.push(
      `candidato alunos do shell deve manter currentPlacement=microfrontend, encontrado ${candidate.currentPlacement}.`,
    );
  }

  if (candidate.targetRemoteName !== manifestItems[0]?.futureRemoteName) {
    errors.push(
      `candidato alunos do shell deve usar targetRemoteName=${manifestItems[0]?.futureRemoteName}, encontrado ${candidate.targetRemoteName}.`,
    );
  }

  errors.push(
    ...compareStringArrays(candidate.runtimeRemoteNames, runtimeRemoteNames, 'candidate.runtimeRemoteNames'),
    ...compareStringArrays(candidate.expectedExposedModules, manifestExposedModules, 'candidate.expectedExposedModules'),
    ...compareStringArrays(candidate.routePaths, manifestPaths, 'candidate.routePaths'),
    ...compareStringArrays(candidate.operationalRoutePaths, operationalRoutePaths, 'candidate.operationalRoutePaths'),
    ...compareStringArrays(candidate.administrativeRoutePaths, administrativeRoutePaths, 'candidate.administrativeRoutePaths'),
    ...compareStringArrays(candidate.landingRoutes, landingRoutes, 'candidate.landingRoutes'),
    ...compareStringArrays(candidate.businessMenuRoutes, businessMenuRoutes, 'candidate.businessMenuRoutes'),
    ...compareStringArrays(candidate.accessMenuRoutes, accessMenuRoutes, 'candidate.accessMenuRoutes'),
    ...compareStringArrays(candidate.contextualRoutes, contextualRoutes, 'candidate.contextualRoutes'),
  );
