...
const errors = [
  ...
  ...validateShellContract(
    manifestItems,
    shellResponsibleRoutes,
    shellResponsibleCandidates,
  ),
];

if (errors.length > 0) {
  console.error('Contrato interno do dominio responsaveis invalido:');
...
console.log(
  `Contrato interno de responsaveis valido: ${manifestItems.length} entradas conferidas entre manifesto, exposes, federation.config.js e shell do host.`,
);
...
function validateRouteClassification(manifestItems) {
  ...
  for (const item of manifestItems) {
    ...
    if (item.path === 'responsibles/:id/edit') {
...
function validateShellExtractionCandidate(
  ...
  if (shellResponsibleCandidates.length !== 1) {
    errors.push(
      `shell do host deve declarar um unico candidato de extracao para responsaveis, encontrados ${shellResponsibleCandidates.length}.`,
    );
    return errors;
  }

  const [candidate] = shellResponsibleCandidates;
  ...
  const runtimeRemoteNames = [
    ...new Set(shellResponsibleRoutes.map(route => route.runtimeRemoteName)),
  ];

  if (candidate.currentPlacement !== 'microfrontend') {
    errors.push(
      `candidato responsaveis do shell deve manter currentPlacement=microfrontend, encontrado ${candidate.currentPlacement}.`,
    );
  }

  if (candidate.targetRemoteName !== manifestItems[0]?.futureRemoteName) {
    errors.push(
      `candidato responsaveis do shell deve usar targetRemoteName=${manifestItems[0]?.futureRemoteName}, encontrado ${candidate.targetRemoteName}.`,
    );
  }
