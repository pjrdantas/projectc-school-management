...
import { DiarioClasseApiService } from './diario-classe-api.service';
import { DIARIO_CLASSE_MOCK } from './diario-classe.mock';
...
export class DiarioClasseComponent implements OnInit {
  ...
  private dataReferenciaIso = this.hojeIso;
  ...
  carregarDiario(): void {
    ...
    this.diarioApi.carregarDiarioClasse({
      idProfessor: '77777777-7777-7777-7777-777777777777',
      idTurma: '88888888-8888-8888-8888-888888888888',
      idDisciplina: '99999999-9999-9999-9999-999999999999',
      anoLetivo: this.obterAnoLetivo(),
      mes: this.obterNumeroMes(),
      dataReferencia: this.dataReferenciaIso
    })
      .pipe(
        map((response) => response.alunos.length ? response : DIARIO_CLASSE_MOCK),
        catchError(() => of(DIARIO_CLASSE_MOCK)),
      )
  ...
  atualizarDiasDoMes(): void {
    ...
    const dataReferencia = this.converterDataAssinatura(this.dataReferenciaIso) ?? this.hoje;
    const dataReferenciaSemHorario = new Date(
    ...
    this.diasDoMes = Array.from({ length: quantidadeDias }, (_, index) => {
      ...
      const diaCorrente = dataIso === this.dataReferenciaIso;
      const passado = data < dataReferenciaSemHorario;
      const futuro = data > dataReferenciaSemHorario;
  ...
  private abrirDialogoImpressao(): void {
    ...
    document.body.classList.add('diario-classe-printing');
    document.title = 'Diário de Classe';
    window.addEventListener(
      'afterprint',
      () => {
        document.body.classList.remove('diario-classe-printing');
  ...
  private preencherTela(response: DiarioClasseResponse): void {
    this.dataReferenciaIso = response.cabecalho.dataAtual || this.dataReferenciaIso;
  ...
  private mapearCabecalho(cabecalho: DiarioClasseCabecalhoResponse): DiarioInfo {
    return {
      ...
      anoLetivo: String(cabecalho.anoLetivo),
      ...
      mes: this.nomeMes(cabecalho.mes),
      data: this.formatarDataBr(this.converterDataAssinatura(cabecalho.dataAtual) ?? this.hoje)
    };
  }
  ...
  private montarRequestSalvamento(): DiarioClasseSaveRequest {
    return {
      ...
      dataLancamento: this.dataReferenciaIso,
  ...
  private diaAtualDoMesSelecionado(): number | null {
    ...
    return ano === anoAtual && mes === mesAtual ? dataReferencia.getDate() : null;
  }
