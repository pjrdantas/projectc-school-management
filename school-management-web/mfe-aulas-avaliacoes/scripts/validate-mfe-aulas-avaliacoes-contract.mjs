...
const errors = [
  ...
  ...validateShellContract(manifestItems, shellTeachingRoutes, shellTeachingCandidates),
];

if (errors.length > 0) {
  console.error('Contrato do mfe-aulas-avaliacoes invalido:');
...
console.log(
  `Contrato do mfe-aulas-avaliacoes valido: ${manifestItems.length} entradas conferidas entre manifesto, exposes locais, federation.config.js e shell do host.`,
);
...
function validateManifestShape(manifestItems) {
  ...
  for (const item of manifestItems) {
    ...
    if ((item.path === 'lessons' || item.path === 'assessments') && item.routeKind !== 'list') {
      errors.push(`rota ${item.path} deve marcar routeKind como list.`);
    }

    if (
      (item.path === 'lessons/:id' || item.path === 'assessments/:id') &&
      item.routeKind !== 'detail'
    ) {
...
function validateShellContract(manifestItems, shellTeachingRoutes, shellTeachingCandidates) {
  return [
    ...validateShellRoutes(manifestItems, shellTeachingRoutes),
    ...validateShellExtractionCandidate(shellTeachingCandidates),
  ];
}
...
function validateShellRoutes(manifestItems, shellTeachingRoutes) {
  ...
  for (const item of manifestItems) {
    ...
    if (shellRoute.runtimeRemoteName !== 'mfe-aulas-avaliacoes') {
      errors.push(
        `rota ${item.path} do shell deve usar runtimeRemoteName mfe-aulas-avaliacoes, encontrado ${shellRoute.runtimeRemoteName}.`,
      );
    }
...
function validateShellExtractionCandidate(shellTeachingCandidates) {
  const errors = [];

  if (shellTeachingCandidates.length !== 0) {
    errors.push(
      `shell do host nao deve mais declarar candidato de extracao para aulas/avaliacoes, encontrados ${shellTeachingCandidates.length}.`,
    );
  }
