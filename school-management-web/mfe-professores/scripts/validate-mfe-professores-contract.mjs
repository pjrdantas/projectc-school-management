...
const errors = [
  ...
  ...validateShellContract(manifestItems, shellProfessorRoutes, shellProfessorCandidates),
];

if (errors.length > 0) {
  console.error('Contrato do mfe-professores invalido:');
...
console.log(
  `Contrato do mfe-professores valido: ${manifestItems.length} entradas conferidas entre manifesto, exposes locais, federation.config.js e shell do host.`,
);
...
function validateManifestShape(manifestItems) {
  ...
  for (const item of manifestItems) {
    ...
    if (item.path === 'teachers' && item.routeKind !== 'list') {
      errors.push(`rota ${item.path} deve marcar routeKind como list.`);
    }

    if (item.path === 'teachers/:id' && item.routeKind !== 'detail') {
...
function validateShellContract(manifestItems, shellProfessorRoutes, shellProfessorCandidates) {
  return [
    ...validateShellRoutes(manifestItems, shellProfessorRoutes),
    ...validateShellExtractionCandidate(shellProfessorCandidates),
  ];
}
...
function validateShellRoutes(manifestItems, shellProfessorRoutes) {
  ...
  for (const item of manifestItems) {
    ...
    if (shellRoute.runtimeRemoteName !== 'mfe-professores') {
      errors.push(
        `rota ${item.path} do shell deve usar runtimeRemoteName mfe-professores, encontrado ${shellRoute.runtimeRemoteName}.`,
      );
    }
...
function validateShellExtractionCandidate(shellProfessorCandidates) {
  const errors = [];

  if (shellProfessorCandidates.length !== 0) {
    errors.push(
      `shell do host nao deve mais declarar candidato de extracao para professores, encontrados ${shellProfessorCandidates.length}.`,
    );
  }
