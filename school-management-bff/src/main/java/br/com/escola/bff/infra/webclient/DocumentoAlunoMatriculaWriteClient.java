...
@Component
public class DocumentoAlunoMatriculaWriteClient extends AbstractDownstreamClientSupport
        implements DocumentoAlunoMatriculaWritePort {
    ...
    public DocumentoAlunoMatriculaWriteClient(
    ...
    public Mono<ResponseEntity<String>> criar(String requestBody, CatalogReadQuery query, AuthSessionContext context) {
        return webClient.post()
                .uri("/internal/v1/documentos-alunos")
    ...
    public Mono<ResponseEntity<String>> enviar(
        ...
        body.part("alunoId", alunoId.toString());
        body.part("tipoDocumento", tipoDocumento);
        if (numeroDocumento != null) {
            body.part("numeroDocumento", numeroDocumento);
        }
        if (observacao != null) {
        ...
        return webClient.post()
                .uri("/internal/v1/documentos-alunos/upload")
                .headers(headers -> addHeaders(headers, query, context))
    ...
    public Mono<ResponseEntity<byte[]>> baixar(
        ...
        return webClient.get()
                .uri("/internal/v1/documentos-alunos/{documentoId}/conteudo", documentoId)
                .headers(headers -> addHeaders(headers, query, context))
    ...
    public Mono<ResponseEntity<String>> excluir(
        ...
        return webClient.delete()
                .uri("/internal/v1/documentos-alunos/{documentoId}", documentoId)
                .headers(headers -> addHeaders(headers, query, context))
                .exchangeToMono(response -> handle(response, INTERNAL_ERROR_MESSAGE))
                .timeout(properties.responseTimeout())
                .onErrorMap(error -> mapTransportError(error, UNAVAILABLE_MESSAGE));
