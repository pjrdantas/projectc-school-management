...
@Component
public class PedagogicalAvaliacaoClient extends AbstractDownstreamClientSupport implements PedagogicalAvaliacaoPort {

    private final WebClient webClient;
    private final PedagogicalServiceClientProperties properties;

    public PedagogicalAvaliacaoClient(
    ...
    public Mono<org.springframework.http.ResponseEntity<String>> listar(
            ...
            CatalogReadQuery query,
            AuthSessionContext context) {
        String uri = UriComponentsBuilder.fromPath("/internal/v1/avaliacoes")
    ...
    public Mono<org.springframework.http.ResponseEntity<String>> buscarPorId(
            ...
            AuthSessionContext context) {
        return webClient.get()
                .uri("/internal/v1/avaliacoes/{id}", avaliacaoId)
                .header("Authorization", query.authorization())
                .header("X-Internal-Token", properties.internalToken())
                .header("X-Correlation-Id", query.correlationId())
                .header("X-Usuario-Id", context.usuarioId().toString())
