...
public class SerieWriteRoutingService implements CreateSerieUseCase {
    ...
    private final MonolithSerieWritePort monolithWritePort;
    private final AcademicCatalogSerieWritePort catalogWritePort;
    private final AcademicCatalogNivelEnsinoResolverPort nivelEnsinoResolverPort;
    ...
    public SerieWriteRoutingService(
            MonolithSerieWritePort monolithWritePort,
            AcademicCatalogSerieWritePort catalogWritePort,
            AcademicCatalogNivelEnsinoResolverPort nivelEnsinoResolverPort,
    ...
    public Mono<SerieCreatedResult> executar(CatalogWriteQuery query, SerieCreateCommand command) {
        ...
        return authContextPort.resolve(new CatalogReadQuery(query.authorization(), query.correlationId()))
                ...
                .flatMap(context -> nivelEnsinoResolverPort.resolve(query, context, command.nivelEnsino())
                        ...
                        .switchIfEmpty(routeToMonolithForUnsupportedNivel(query, command)))
                .doOnError(error -> observabilityPort.recordCatalogFailure(decision, error));
    }

    private Mono<SerieCreatedResult> routeToMonolithForUnsupportedNivel(
