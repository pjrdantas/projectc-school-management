...
class DashboardAlertaReadProxyIntegrationTest {
    ...
    @Test
    void deveConsumirDashboardQueryServiceNaLeituraOficialDeAlertas() throws InterruptedException {
        ...
        DASHBOARD_QUERY.enqueue(new MockResponse()
                .setHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                .setHeader(TrustedHeaders.CORRELATION_ID, "corr-dashboard-alerta-1")
                .setBody("""
                        [{
        ...
        client.get().uri(uriBuilder -> uriBuilder.path("/api/dashboard/alertas")
                ...
                .header(TrustedHeaders.CORRELATION_ID, "corr-dashboard-alerta-1")
                .exchange()
                .expectStatus().isOk()
                .expectHeader().valueEquals(TrustedHeaders.CORRELATION_ID, "corr-dashboard-alerta-1")
                .expectBody()
        ...
        assertThat(dashboardRequest.getPath()).isEqualTo("/internal/v1/dashboard/alertas?publicoCodigo=diretor");
        ...
        if (monolithRequest != null) {
            assertThat(monolithRequest.getPath()).isNotEqualTo("/api/dashboard/alertas?publicoCodigo=diretor");
        }
    }

    @Test
    void deveConsumirDashboardQueryServiceNaLeituraOficialDeAlertasDoProfessor() throws InterruptedException {
        ...
        client.get().uri(uriBuilder -> uriBuilder.path("/api/dashboard/alertas")
                        .queryParam("publicoCodigo", "professor")
                        .queryParam("professorId", professorId)
                        .build())
                .header(HttpHeaders.AUTHORIZATION, "Bearer opaque-token")
                .header(TrustedHeaders.CORRELATION_ID, "corr-dashboard-alerta-2")
                .exchange()
                .expectStatus().isOk()
                .expectBody()
        ...
        assertThat(dashboardRequest.getPath()).isEqualTo(
                "/internal/v1/dashboard/alertas?publicoCodigo=professor&professorId=" + professorId);
    }
