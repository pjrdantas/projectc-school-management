...
class AlunoAtualizacaoControllerTest {

    @Test
    void deveExporAtualizacaoInternaComContratoCompativel() throws Exception {
        UUID alunoId = UUID.randomUUID();
        UUID escolaId = UUID.randomUUID();
        InternalRequestContext context = new InternalRequestContext(
                "corr-aluno-update", UUID.randomUUID(), escolaId);
        AtomicReference<InternalRequestContext> receivedContext = new AtomicReference<>();
        AtualizarAlunoUseCase useCase = (id, request, internalContext) -> {
            receivedContext.set(internalContext);
            return new AlunoAtualizado(
                    id, request.nomeCompleto(), request.cpf(), request.email(), request.telefone(),
        ...
        MockMvc mockMvc = MockMvcBuilders.standaloneSetup(new AlunoAtualizacaoController(useCase))
                .setControllerAdvice(new ApiExceptionHandler())
                .build();

        mockMvc.perform(put("/internal/v1/alunos/{alunoId}", alunoId)
                ...
                .andExpect(jsonPath("$.nomeCompleto").value("Aluno Atualizado"))
