...
import br.com.escola.peopleservice.application.port.out.EnderecoWritePort;
...
@ConditionalOnProperty(
        ...
        havingValue = "true")
public class OrigemAtualPessoaAddressWriteClient implements EnderecoWritePort {
    ...
    public OrigemAtualPessoaAddressWriteClient(RestClient monolithPeopleRestClient, MeterRegistry meterRegistry) {
        ...
        try {
            OrigemAtualAddressWriteResponse response = restClient.put()
                    ...
                    .body(new OrigemAtualAddressWriteRequest(
                            ...
                            command.uf()))
                    .retrieve()
                    .body(OrigemAtualAddressWriteResponse.class);
    ...
    public PessoaEnderecoWriteResult removerEnderecosDaPessoa(PessoaEnderecoCleanupCommand command) {
        validar(command.commandId(), command.pessoaId(), command.escolaId(), command.idempotencyKey(), command.requestedBy());
        try {
            OrigemAtualAddressWriteResponse response = restClient.delete()
    ...
    private PessoaEnderecoWriteResult toResult(
            UUID commandId,
            UUID pessoaId,
            OrigemAtualAddressWriteResponse response) {
        if (response == null) {
    ...
    private record OrigemAtualAddressWriteRequest(
    ...
    private record OrigemAtualAddressWriteResponse(
            String commandId,
            UUID pessoaId,
            UUID enderecoId,
